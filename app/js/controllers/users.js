...
angular.module('app.controllers.Users', []).controller('UsersCtrl', function ($scope, $rootScope, $mdDialog, UsersService, Connection) {
  ...
  $scope.getTotal = function () {
    UsersService.getTotal(db).then(function (total) {
      $scope.total = total;
    }, function (err) {
      console.log(err);
    });
  ...
  $scope.getList = function (limit, offset) {
    ...
    UsersService.getMembers(db, limit, offset).then(function (rows) {
      $scope.members = rows;
  ...
  $scope.remove = function (id) {
    var confirm = $mdDialog.confirm().title('ยืนยันการลบข้อมูล').textContent('คุณต้องลบรายการนี้ ใช่หรือไม่?').ariaLabel('confirm')
    // .targetEvent(event)
    .ok('ใช่, ฉันต้องการลบ').cancel('ยกเลิก');

    $mdDialog.show(confirm).then(function () {
      // ยืนยันการลบ
      UsersService.remove(db, id).then(function () {
        $scope.initialData();
      }, function (err) {
