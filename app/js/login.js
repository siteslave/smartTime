...
angular.module('app', ['ngMaterial', 'ui.router', 'app.services.Connnection', 'app.controllers.Settings']).config(function ($mdThemingProvider, $stateProvider, $urlRouterProvider) {
  ...
  $scope.Login = function () {
    ...
    LoginService.doLogin(db, $scope.username, encryptedPass).then(function (total) {
      if (total) {
  ...
  return {
    doLogin: function doLogin(db, username, password) {
      var q = $q.defer();
      db('admin').where('username', username).where('password', password).count('* as total').then(function (rows) {
